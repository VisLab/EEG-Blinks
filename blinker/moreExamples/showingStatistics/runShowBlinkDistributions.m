...
figure ('Color', [1, 1, 1], 'Name', 'Good blinks/min')
hold on
for k = 1:numExp
...
figure ('Color', [1, 1, 1], 'Name', 'Good blink duration HB')
hold on
for k = 1:numExp
...
hold off
box on
xlabel('Duration HB (s) for prototypical blinks')
...
figure ('Color', [1, 1, 1], 'Name', 'Blink duration HB')
hold on
for k = 1:numExp
...
figure ('Color', [1, 1, 1], 'Name', 'Good blink duration HZ')
hold on
for k = 1:numExp
   durationGHZ = experiments(k).durationHZ(:, 3);
...
hold off
box on
xlabel('Duration HZ (s) for prototypical blinks')
legend(legendHandlesGHZ, legendEntriesGHZ, 'Location', 'SouthEast', ...
      'Interpreter', 'None')
set(gca, 'XLim', [0.05, 0.35])

%% Output the mean and standard deviation of the goood half zero blinks
fprintf('Good blink half zero durations\n');
for k = 1:numExp
   durationGHZ = experiments(k).durationHZ(:, 3);
   fprintf('%s: mean = %g, std = %d\n', experiments(k).experiment, ...
       nanmean(durationGHZ), nanstd(durationGHZ));
end
%% Blink duration half zero
theHandlesHZ = cell(numExp, 1);
legendHandlesHZ = zeros(1, numExp);
legendEntriesHZ = cell(1, numExp);
figure ('Color', [1, 1, 1], 'Name', 'Blink duration HZ')
hold on
for k = 1:numExp
   durationHZ = experiments(k).durationHZ(:, 1);
...
figure ('Color', [1, 1, 1], 'Name', 'Good aPVZ')
hold on
for k = 1:numExp
...
figure ('Color', [1, 1, 1], 'Name', 'Good NAPRV')
hold on
for k = 1:numExp
